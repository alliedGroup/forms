...
async function fetchCaptureByOptions() {
    await fetchCsvData();
    const captureByOptions = [...new Set(csvData.slice(1).map(row => row[13].trim()).filter(Boolean))];
...
async function fetchAllocatedOptions() {
    await fetchCsvData();
    const allocatedToOptions = [...new Set(csvData.slice(1).map(row => row[13].trim()).filter(Boolean))];
